"""CoverageAgent invocation — same shape as HygieneAgent."""
...
from src.wazuh_health.tools.readonly import COVERAGE_TOOLS

_PROMPT = (Path(__file__).parent / "prompts" / "coverage.md").read_text()


def invoke_coverage_agent(
    *, hits, audit_store: AuditStore, findings_store: FindingsStore,
    light_model: str, now: datetime,
) -> list[int]:
    invocation = AgentInvocation(
        agent_name="CoverageAgent", instructions=_PROMPT, tools=COVERAGE_TOOLS,
    ...
    for raw in (findings or []):
        try:
            ...
            continue
        hk = compute_hash_key("coverage", metric=str(hits[0].metric if hits else ""), evidence=clean.evidence)
        persisted.append(findings_store.upsert(clean, hash_key=hk))
    audit_store.record_agent_run(
        agent="CoverageAgent", started_at=now, ended_at=now, status="ok",
